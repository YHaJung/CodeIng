...
def CBRS(request):
    try:
        ...
        r, c = recommend.shape
        if pk < r:
            krecommend = np.argsort(-recommend[pk])[10 * page - 10: 10 * page]
            ...
            for lectureidx in krecommend:
                ...
                if lectureidx in lectures:
                    ...
                    i = Lecture.objects.filter(lectureidx=lectureidx).values('lectureidx', 'lecturename', 'thumburl',
                                                                             'lecturer', 'level', 'price', 'rating',
                                                                             'level__levelidx', 'level__levelname',
                                                                             'siteinfo',
                                                                             'siteinfo__logoimage').distinct()
            ...
            krecommend = np.argsort(-recommend)[:, 2 * page - 2: 2 * page]
...
def CBRSlist(request):
    ...
    r, c = recommend.shape
    if pk < r:
        krecommend = np.argsort(-recommend[int(pk)])[:30]
        num = 0
        for lectureidx in krecommend:
        ...
        recommend = pickle.load(open('knn_models/recommend.pkl', 'rb'))
        nneigh = 25
        krecommend = np.argsort(-recommend)[:,:4]
        cnt = Counter(krecommend.flatten())  # age_C데이터를 카운트한다.
        krecommend = cnt.most_common()
...
def KNN_IBCF(request, pk=None):
    ...
    for x in range(0, len(distances.flatten())):
        i = Lecture.objects.filter(lectureidx=indices.flatten()[x]).values('siteinfo__logoimage', 'thumburl',
                                                                           'lecturer',
                                                                           'level', 'price', 'rating',
                                                                           'level__levelidx', 'level__levelname',
                                                                           'siteinfo').distinct()
...
def KNN_UBCF(request, pk=None):
    ...
    for x in range(0, len(distances.flatten())):
        i = Lecture.objects.filter(lectureidx=indices.flatten()[x]).values('siteinfo__logoimage', 'lectureidx',
                                                                           'lecturename', 'thumburl',
                                                                           'lecturer',
                                                                           'level', 'price', 'rating',
                                                                           'level__levelidx',
                                                                           'siteinfo').distinct()
